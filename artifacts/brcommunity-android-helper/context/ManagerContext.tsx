...
import { captureException, setTelemetryContext, trackEvent } from '@/lib/telemetry';
import {
  closeChromeTab,
...
async function saveActivityItem(message: string, ok: boolean): Promise<Activity> {
  const item = { id: `${Date.now()}-${Math.random().toString(36).slice(2, 7)}`, message, ok, at: Date.now() };
  const current = JSON.parse((await AsyncStorage.getItem(ACTIVITY_KEY)) || '[]') as Activity[];
...
export function ManagerProvider({ children }: { children: React.ReactNode }) {
  ...
  const record = useCallback(async (message: string, ok = true) => {
    const item = await saveActivityItem(message, ok);
    setActivity((current) => [item, ...current].slice(0, MAX_ACTIVITY));
  }, []);

  const refreshActivity = useCallback(async () => {
    setActivity(JSON.parse((await AsyncStorage.getItem(ACTIVITY_KEY)) || '[]') as Activity[]);
  }, []);
  ...
  useEffect(() => {
    void (async () => {
      const stored = JSON.parse((await AsyncStorage.getItem(STATE_KEY)) || '{}') as Partial<{
        type: ContentType; scope: Scope; max: number; config: RemoteConfig; queue: LinkItem[];
        items: ManagedItem[]; sessionOpened: number; soundEnabled: boolean; lastError: string; sessionId: string; globalStopVersion: number; lastHeartbeatAt: number; status: ManagerStatus; nextOpenAt: number | null;
        browserMode: BrowserMode; openMode: OpenMode; autoOpen?: boolean;
      }>;
      if (stored.type) setTypeState(stored.type);
      if (stored.scope) setScopeState(stored.scope);
      if (stored.max) setMaxState(stored.max);
      if (stored.config) {
      ...
      const now = Date.now();
      const restoredItems = stored.items?.filter((item) => item.closeAt > now) || [];
      if (stored.items) {
        itemsRef.current = restoredItems;
        setItems(restoredItems);
        const expiredItems = stored.items.filter((item) => item.closeAt <= now);
      ...
      if (stored.sessionId) sessionId.current = stored.sessionId;
  ...
  const heartbeat = useCallback(async (nextStatus: 'running' | 'stopped') => {
    ...
    void trackEvent('manager.heartbeat', { status: nextStatus, duration_ms: heartbeatAt - heartbeatStartedAt });
  ...
  const openNext = useCallback(async () => {
    ...
    try {
      ...
      captureException(error, { operation: 'manager.open_next' });
      void trackEvent('manager.open_failed', {}, 'error');
      ...
      void trackEvent('manager.open_cycle', { generation });
      queueReservationRef.current = null;
  ...
  const engage = useCallback(async (localId: string) => {
    ...
    if (!item || item.engagementSent || Date.now() >= item.closeAt) return;
    void trackEvent('manager.engagement_requested', {});
    let accepted = true;
  ...
  const removeExpired = useCallback(async (localId: string) => {
    const item = itemsRef.current.find((candidate) => candidate.localId === localId);
    if (!item) return;
    void trackEvent('manager.expiration', {});
    if (browserModeRef.current === 'chrome') {
      const closed = await closeChromeTab(localId, item.url).catch(() => false);
      if (closed) await cancelChromeTabLifecycle(localId).catch(() => {});
      // A refused close is intentionally not retried from React state. The native
      // lifecycle remains registered and will retry when the target becomes
      // verifiable again. The UI item can safely leave the active set.
    }
  ...
  const start = useCallback(async () => {
    ...
    try {
      ...
      setTelemetryContext({ browser_mode: browserModeRef.current, content_type: typeRef.current, scope: scopeRef.current });
      void trackEvent('manager.started', { browser_mode: browserModeRef.current, content_type: typeRef.current, scope: scopeRef.current });
  ...
  }, [heartbeat, loadQueue, openNext, persist, record, refreshChromeStatus, status, user]);
  ...
  const stop = useCallback(async () => {
    ...
    statusRef.current = 'STOPPED';
    setStatus('STOPPED');
    void trackEvent('manager.stopped', {});

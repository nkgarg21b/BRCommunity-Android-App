...
public class ChromeAccessibilityService extends AccessibilityService {
  ...
  private static final long ACTION_VERIFY_GRACE_MS = 350L;
  ...
  public void scheduleTab(String id, String expectedUrl, long closeAt, long engageAt) {
    String runtimeId = NativeChromeScheduler.get(this).getActiveRuntimeId();
    if (TextUtils.isEmpty(runtimeId)) throw new IllegalStateException("Native Chrome runtime is not active.");
    NativeChromeScheduler.get(this).schedule(runtimeId, id, expectedUrl, closeAt, engageAt, false, System.nanoTime());
  }
  ...
  /** Called only by NativeChromeScheduler after an AlarmManager authorization check. */
  public void handleNativeAlarm(String id, String reason, long token) {
    ...
    if (!entry.engaged && entry.engageAt > 0 && now >= entry.engageAt && now < entry.closeAt) {
      if (engageTabInternal(entry.id, entry.url)) scheduler.markEngaged(entry.id, entry.token);
    }
    if (now >= entry.closeAt) {
      if (closeTabInternal(entry.id, entry.url)) scheduler.cancel(entry.id);
  ...
  public boolean engageTab(String sessionId, String expectedUrl) {
    ...
    if (entry == null || !entry.url.equals(ChromeSessionPolicy.normalizeUrl(expectedUrl)) || !scheduler.isRuntimeActiveFor(sessionId)) return false;
    return engageTabInternal(sessionId, entry.url);
  }
  ...
  public boolean closeTab(String sessionId, String expectedUrl) {
    ...
    NativeChromeScheduler.Snapshot entry = scheduler.get(sessionId);
    if (entry == null || !entry.url.equals(ChromeSessionPolicy.normalizeUrl(expectedUrl)) || !scheduler.isRuntimeActiveFor(sessionId)) return false;
    return closeTabInternal(sessionId, entry.url);
  }

  private boolean engageTabInternal(String id, String expectedUrl) {
  ...
  private boolean closeTabInternal(String id, String expectedUrl) {
    SurfaceSnapshot surface = verifySurface(expectedUrl, -1, null);
    if (surface == null) return false;
    AccessibilityNodeInfo root = getRootForWindow(surface.windowId);
    if (root == null) return false;
  ...
  private SurfaceSnapshot inspectSurface(AccessibilityNodeInfo root) {
    String raw = findChromeUrl(root);
    String normalized = ChromeSessionPolicy.normalizeUrl(raw);
  ...
  static String normalizeUrl(String value) { return ChromeSessionPolicy.normalizeUrl(value); }
  static boolean urlMatches(String expected, String observed) { return ChromeSessionPolicy.urlMatches(expected, observed); }
}

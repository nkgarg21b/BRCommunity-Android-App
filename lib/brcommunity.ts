...
export async function apiJson<T>(path: string, options: RequestInit = {}): Promise<T> {
  ...
  const timeoutMs = path.includes('/extension/heartbeat') ? 10_000 : 15_000;
  ...
  try {
    ...
    void trackEvent(timedOut ? 'api.timeout' : 'api.network_error', { method: options.method || 'GET', duration_ms: Date.now() - startedAt, timeout_ms: timeoutMs }, 'error');
    if (!callerSignal?.aborted) captureException(error, { operation: timedOut ? 'api.timeout' : 'api.request', method: options.method || 'GET' });
    if (callerSignal?.aborted) throw error;
    throw new Error(timedOut ? `BRCommunity request timed out after ${timeoutMs / 1000}s.` : `Unable to connect to BRCommunity. Check your connection.`);
  } finally {
